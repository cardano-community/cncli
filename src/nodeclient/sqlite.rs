...
impl SQLiteBlockStore {
    ...
    pub fn new(db_path: &PathBuf) -> Result<SQLiteBlockStore, Error> {
        ...
        let tx = db.transaction()?;
        {
            ...
            tx.execute("CREATE TABLE IF NOT EXISTS db_version (version INTEGER PRIMARY KEY)", NO_PARAMS)?;
            ...
            if version < 1 {
                ...
                tx.execute("CREATE TABLE IF NOT EXISTS chain (\
                ...
                tx.execute("CREATE INDEX IF NOT EXISTS idx_chain_slot_number ON chain(slot_number)", NO_PARAMS)?;
                tx.execute("CREATE INDEX IF NOT EXISTS idx_chain_orphaned ON chain(orphaned)", NO_PARAMS)?;
                tx.execute("CREATE INDEX IF NOT EXISTS idx_chain_hash ON chain(hash)", NO_PARAMS)?;
                tx.execute("CREATE INDEX IF NOT EXISTS idx_chain_block_number ON chain(block_number)", NO_PARAMS)?;
            }

            // Upgrade their database to version 2
            if version < 2 {
                info!("Upgrade database to version 2...");
                tx.execute("CREATE TABLE IF NOT EXISTS slots (\
                    id INTEGER PRIMARY KEY AUTOINCREMENT, \
            ...
            if version < 3 {
                ...
                tx.execute("CREATE INDEX IF NOT EXISTS idx_chain_node_vkey ON chain(node_vkey)", NO_PARAMS)?;
                tx.execute("ALTER TABLE chain ADD COLUMN pool_id TEXT NOT NULL DEFAULT ''", NO_PARAMS)?;
                tx.execute("CREATE INDEX IF NOT EXISTS idx_chain_pool_id ON chain(pool_id)", NO_PARAMS)?;

                let count: i64 = tx.query_row("SELECT COUNT(DISTINCT node_vkey) from chain", NO_PARAMS, |row| Ok(row.get(0)?))?;

                if count > 0 {
                    let mut stmt = tx.prepare("SELECT DISTINCT node_vkey FROM chain")?;
                    let vkeys = stmt.query_map(NO_PARAMS, |row| {
                    ...
                    for (i, node_vkey) in vkeys.into_iter().enumerate() {
                        ...
                        let pool_id = hex::encode(Params::new().hash_length(28).to_state().update(&*node_vkey_bytes).finalize().as_bytes().to_vec());

                        tx.execute_named("UPDATE chain SET pool_id=:pool_id WHERE node_vkey=:node_vkey",
                                         named_params! {
                            ":pool_id": pool_id,
                            ":node_vkey": vkey
                        })?;
            ...
            if version < 0 {
                tx.execute("INSERT INTO db_version (version) VALUES (?1)", &[&SQLiteBlockStore::DB_VERSION])?;
            } else {
    ...
    fn sql_save_block(&mut self, pending_blocks: &mut Vec<BlockHeader>, network_magic: u32) -> Result<(), rusqlite::Error> {
        ...
        { // scope for db transaction
            ...
            for block in pending_blocks.drain(..) {
                ...
                let pool_id = Params::new().hash_length(28).to_state().update(&*block.node_vkey).finalize().as_bytes().to_vec();
