...
pub(crate) fn sodium_crypto_vrf_prove(secret_key: &Vec<u8>, seed: Vec<u8>) -> Result<Vec<u8>, String> {
    ...
    unsafe {
        let rc = crypto_vrf_prove(proof.as_mut_ptr(), secret_key.as_ptr(), seed.as_ptr(), seed.len() as u64);
        if rc != 0 {
            Err(format!("libsodium crypto_vrf_prove() failed, returned {}, expected 0", rc))
        } else {
            proof.set_len(80);
            Ok(proof)
        }
...
pub(crate) fn sodium_crypto_vrf_proof_to_hash(proof: Vec<u8>) -> Result<Vec<u8>, String> {
    ...
    unsafe {
        ...
        if rc != 0 {
            Err(format!("libsodium crypto_vrf_proof_to_hash() failed, returned {}, expected 0", rc))
        } else {
            hash.set_len(64);
            Ok(hash)
        }
